...
# Setup infrastructure (BigQuery, GCS, Vertex AI Search)
infra:
	@PROJECT_ID=$${PROJECT_ID:-$$(gcloud config get-value project 2>/dev/null)}; \
	if [ -z "$$PROJECT_ID" ] || [ "$$PROJECT_ID" = "(unset)" ]; then \
		echo "❌ ERROR: No GCP project configured"; \
		echo ""; \
		echo "Please set your GCP project first:"; \
		echo "  gcloud config set project YOUR-PROJECT-ID"; \
		echo ""; \
		echo "Or set PROJECT_ID environment variable:"; \
		echo "  export PROJECT_ID=your-project-id && make infra"; \
		exit 1; \
	fi; \
	echo "🏗️  Setting up infrastructure for project: $$PROJECT_ID"; \
	cd infra && $(MAKE) infra PROJECT_ID=$$PROJECT_ID
