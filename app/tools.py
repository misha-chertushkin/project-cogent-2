...
def search_documents(
    ...
    try:
        ...
        request = discoveryengine.SearchRequest(
            ...
            # Enable summary and snippets (Standard Edition compatible)
        ...
        # Debugging: Log the full summary object and text
        logger.debug(f"Full summary object: {response.summary}")
        if hasattr(response, "summary") and response.summary.summary_text:
            logger.debug(f"Summary text: {response.summary.summary_text}")
            output_lines.append(f"Summary Answer: {response.summary.summary_text}\n")
            output_lines.append("-" * 40 + "\n")
        else:
            logger.debug("No summary text found in response.")
        ...
        for result in results:
            ...
            if hasattr(doc, 'derived_struct_data') and doc.derived_struct_data:
                ...
                # Fallback: show raw derived data preview
                    ...
                    # Convert MapComposite to dict for readable output
                    preview = str(dict(derived))[:300]
